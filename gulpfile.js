...
gulp.task("style", function() {
  gulp.src("source/less/style.less") //берем источник
...
gulp.task("serve", function() {
  server.init({
    server: "source/" // путь до корня сайта (index.html) .тажа самая папка в которой мы находимся
  });

  gulp.watch("source/less/**/*.less", ["style"]) // файлы за которыми надо следить **любое количесвто подпапок, *любое. Если есть изменения то запускает задачу Style gulp.task("style")
  gulp.watch("source/*.html", ["html"]) // тоже самое только для html
});

//оптимизирует картинки, верезает информацию
gulp.task("images", function() {
  return gulp.src("source/img/**/*.{png,jpg,svg}") //форматы с которые нужно оптимизировать
  .pipe(imagemin([
  ...
  .pipe(gulp.dest("build/img")); //положить в папку img
});
